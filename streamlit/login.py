...
subpage = st.segmented_control(
    "Select subpage",
    ["Login", "Register", "Forgot credentials", "Update details"],
    default="Login",
...
def login_page() -> None:
    if st.session_state.get("authentication_status"):
        ...
        if st.session_state.get("roles", None) == "admin":
            st.success("Welcome Admin. You are successfully logged in!")
        else:
            st.success(
                f"Welcome *{st.session_state.get('username')}*. You are successfully logged in!",
            )
    else:
        try:
    ...
    st.write("""
            ...
            - update team rosters
            - delete open game rooms""")


def update_page() -> None:
    def update_user_in_db() -> None:
        current_data = st.session_state["user_data"]["usernames"][
...
def register_page() -> None:
    try:
        ...
        def update_user_in_db(username: str) -> None:
        ...
        (
            ...
            _name_of_registered_user,
        ) = st.session_state["authenticator"].register_user(
            two_factor_auth=True,
            password_hint=False,
            fields={"Password": "Password (will be hashed)"},
        )
...
def forgot_page() -> None:
    forgotten = st.segmented_control(
        "Choose the forgotten credential",
        ["Password", "User name"],
    )

    def forgotten_username() -> None:
        try:
    ...
    def forgotten_password() -> None:
        try:
            (
                username_of_forgotten_password,
                email_of_forgotten_password,
                _password_of_forgotten_password,
