...
from sources.plots import (
    display_tree,
    plot_play_probabilities,
    plot_points,
...
def create_random_play_data(season: int, sql_engine: Engine) -> None:
    teams = pd.DataFrame(
        query_db(
            sql_engine,
            "SELECT abbreviation, name FROM teams WHERE team_id NOT IN (-2, -1, 31, 32, 38);",
        ),
    )
    home_team = teams.sample(1).iloc[0].to_dict()
    away_team = (
        teams.loc[teams["abbreviation"] != home_team["abbreviation"]]
        .sample(1)
        .iloc[0]
        .to_dict()
    )
    week = random.SystemRandom().randint(1, 18)
    quarter = random.SystemRandom().randint(1, 4)
    clock_seconds = random.SystemRandom().randint(0, 900)
    total_time_left = (4 - quarter) * 900 + clock_seconds

    offense_at_home = random.SystemRandom().choice([0, 1])

    down = random.SystemRandom().randint(1, 4)
    distance = random.SystemRandom().randint(1, 10)
    yards_to_endzone = random.SystemRandom().randint(1, 99)

    # Score
    home_score = random.SystemRandom().randint(0, 35)
    away_score = random.SystemRandom().randint(0, 35)
    score_diff = home_score - away_score

    # Standings generation
    home_wins = random.SystemRandom().randint(0, 17)
    home_losses = week - home_wins - random.SystemRandom().randint(0, 1)
    away_wins = random.SystemRandom().randint(0, 17)
    away_losses = week - away_wins - random.SystemRandom().randint(0, 1)

    home_home_wins = random.SystemRandom().randint(0, home_wins) if home_wins > 0 else 0
    home_road_wins = home_wins - home_home_wins
    home_home_losses = (
        random.SystemRandom().randint(0, home_losses) if home_losses > 0 else 0
    )
    home_road_losses = home_losses - home_home_losses

    away_home_wins = random.SystemRandom().randint(0, away_wins) if away_wins > 0 else 0
    away_road_wins = away_wins - away_home_wins
    away_home_losses = (
        random.SystemRandom().randint(0, away_losses) if away_losses > 0 else 0
    )
    ...
    play_data = {
        ...
        **standings,
        ...
        "completionRate": round(random.SystemRandom().uniform(0.5, 0.75), 2),
        "passToRushRatio": round(random.SystemRandom().uniform(0.8, 1.5), 2),
    }
...
def display_buttons(position: str) -> None:
    if st.button("Update latest play", key=f"restart{position}"):
        with st.spinner("Updating latest play..."):
            update_running_game(play_data["game_id"], sql_engine)
        st.rerun()
    st.write(
        "Updating the latest play relies on the update of the ESPN database. In case they are falling behind, feel free to modify the game situation manually.",
    )
...
if ("game" in st.query_params) and (st.query_params["game"] != "None"):
    raw_game = st.query_params.get("game")
    try:
        game_id = int(raw_game)
    except (TypeError, ValueError):
        st.error("Invalid game id in query params.")
    else:
        selected_rows = query_db(
            sql_engine,
            "SELECT name FROM games WHERE game_id = :game_id",
            game_id=game_id,
        )
        if selected_rows:
            selected_game = selected_rows[0]["name"]
...
choice = st.segmented_control(
    "Choose an option:",
    ["Live Game", "User Input (Full)", "User Input (Tree)"],
...
if st.session_state["choice"] == "Live Game":
    ...
    if "game_name_selected" not in st.session_state:
        st.session_state["game_name_selected"] = None
    st.session_state["game_name_selected"] = st.selectbox(
        "Games",
        options=[i["name"] for i in running_games],
        index=None
        if st.session_state["game_name_selected"] is None
        else [i["name"] for i in running_games].index(
            st.session_state["game_name_selected"],
        ),
    ...
    if len(running_games) == 0:
        st.error(
            "There isn't any game running right now. Please check back when the next one begins.",
        )
    ...
    if "play_data" in st.session_state:
        ...
        play_data = {
            ...
            "awayScore": 0,
            "scoreDiff": 0,
            "standing_home_overall_win": 0,
            "standing_home_home_win": 0,
            "standing_home_road_win": 0,
            "standing_home_overall_loss": 0,
            "standing_home_home_loss": 0,
            "standing_home_road_loss": 0,
            "standing_away_overall_win": 0,
            "standing_away_home_win": 0,
            "standing_away_road_win": 0,
            "standing_away_overall_loss": 0,
            "standing_away_home_loss": 0,
            "standing_away_road_loss": 0,
            "homeAbr": "ATL",
            "awayAbr": "BUF",
...
if (st.session_state["choice"] == "User Input (Full)") or (
    (st.session_state["choice"] == "Live Game") and (play_data["next_down"] > 0)
):
    ...
    with col1:
        game_type = st.radio(
            "Game Type",
            ["Regular", "Postseason"],
            index=0 if play_data["game_type"] == "regular-season" else 1,
            disabled=(st.session_state["choice"] == "Live Game"),
        )
    with col2:
        season = st.selectbox(
            "Season",
            options=range(2009, play_data["season"] + 1),
            index=list(range(2009, play_data["season"] + 1)).index(play_data["season"]),
            disabled=(st.session_state["choice"] == "Live Game"),
        )
        week_options = (
            list(range(1, 19))
            if game_type == "Regular"
            else ["SuperBowl", "ConfChamp", "DivRound", "WildCard"]
        )
        try:
            week = st.selectbox(
                "Week",
                options=week_options,
                index=week_options.index(play_data["week"]),
                disabled=(st.session_state["choice"] == "Live Game"),
            )
        except:
            week = st.selectbox(
    ...
    team_to_abr = {
        ...
        for i in query_db(sql_engine, "SELECT abbreviation, name FROM teams;")
    }
    team_to_color = {
        i["name"]: i["color"]
        for i in query_db(sql_engine, "SELECT color, name FROM teams;")
    }
    ...
    with col1:
        home_team = st.selectbox(
            "Select Home Team",
            options=all_teams,
            index=all_teams.index(play_data["homeName"]),
            disabled=(st.session_state["choice"] == "Live Game"),
        )
    ...
    with col2:
        away_team = st.selectbox(
            "Select Away Team",
            options=all_teams,
            index=all_teams.index(play_data["awayName"]),
            disabled=(st.session_state["choice"] == "Live Game"),
        )
    ...
    col1, col2, _ = st.columns(3)
    with col1:
        team_possession = st.radio(
            "Team in Possession",
            options=[home_team, away_team],
            index=1
            if (play_data["offenseAtHome"] == play_data["possessionChange"])
            else 0,
            disabled=(st.session_state["choice"] == "Live Game"),
        )
        yards_to_endzone = st.slider(
            "Yards to Endzone",
            min_value=1,
            max_value=100,
            step=1,
            value=play_data["next_yardsToEndzone"],
            disabled=(st.session_state["choice"] == "Live Game"),
        )
    with col2:
        down = st.slider(
            ...
            max_value=4,
            step=1,
            value=play_data["next_down"],
            disabled=(st.session_state["choice"] == "Live Game"),
        )
        yards_for_first = st.number_input(
            "Yards for 1st",
            min_value=1,
            step=1,
            value=play_data["next_distance"],
            disabled=(st.session_state["choice"] == "Live Game"),
        )
    ...
    if standings_input == "No":
        ...
        with col1:
            standing_home_overall_win = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_home_overall_win"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
            standing_home_overall_loss = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_home_overall_loss"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
            standing_home_home_win = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_home_home_win"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
            standing_home_home_loss = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_home_home_loss"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
            standing_home_road_win = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_home_road_win"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
            standing_home_road_loss = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_home_road_loss"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
        ...
        with col2:
            ...
            standing_away_overall_win = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_away_overall_win"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
            standing_away_overall_loss = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_away_overall_loss"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
            standing_away_home_win = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_away_home_win"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
            standing_away_home_loss = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_away_home_loss"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
            standing_away_road_win = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_away_road_win"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
            standing_away_road_loss = st.number_input(
                ...
                min_value=0,
                step=1,
                value=play_data["standing_away_road_loss"],
                disabled=(st.session_state["choice"] == "Live Game"),
            )
    else:
    ...
    prediction_data = {
        "quarter": quarter,
        "clock_seconds": clock_seconds,
        "offenseAtHome": 1 if team_possession == home_team else 0,
        "down": down,
        "distance": yards_for_first,
        ...
        "standing_offense_home_win": standing_home_home_win
        if team_possession == home_team
        else standing_away_home_win,
        "standing_offense_road_win": standing_home_road_win
        if team_possession == home_team
        else standing_away_road_win,
        "standing_offense_overall_loss": standing_home_overall_loss
        if team_possession == home_team
        else standing_away_overall_loss,
        "standing_offense_home_loss": standing_home_home_loss
        if team_possession == home_team
        else standing_away_home_loss,
        "standing_offense_road_loss": standing_home_road_loss
        if team_possession == home_team
        else standing_away_road_loss,
        "standing_defense_overall_win": standing_away_overall_win
        if team_possession == home_team
        else standing_home_overall_win,
        "standing_defense_home_win": standing_away_home_win
        if team_possession == home_team
        else standing_home_home_win,
        "standing_defense_road_win": standing_away_road_win
        if team_possession == home_team
        else standing_home_road_win,
        "standing_defense_overall_loss": standing_away_overall_loss
        if team_possession == home_team
        else standing_home_overall_loss,
        "standing_defense_home_loss": standing_away_home_loss
        if team_possession == home_team
        else standing_home_home_loss,
        "standing_defense_road_loss": standing_away_road_loss
        if team_possession == home_team
        else standing_home_road_loss,
        ...
        "offenseAbr": team_to_abr[home_team]
        if team_possession == home_team
        else team_to_abr[away_team],
        "defenseAbr": team_to_abr[away_team]
        if team_possession == home_team
        else team_to_abr[home_team],
        "totalTimeLeft": (clock_seconds + (4 - quarter) * 15 * 60),
        "completionRate": play_data["completionRate"],
        "passToRushRatio": play_data["passToRushRatio"],
    }

    pkl_path = Path("streamlit/sources/nn_classifier.pkl")
    with pkl_path.open("rb") as f:
        nn_classifier = dill.load(f)  # TODO: replace with joblib # noqa: S301
    pkl_path = Path("streamlit/sources/nn_encoder.pkl")
    with pkl_path.open("rb") as f:
        nn_encoder = dill.load(f)  # TODO: replace with joblib # noqa: S301

    try:
        class_probabilities = nn_classifier.predict_proba(
            pd.DataFrame.from_dict(prediction_data, orient="index").T,
        )
    except:
        st.rerun()
    ...
    (
        col1,
        col2,
    ) = st.columns(2, vertical_alignment="top", gap="large")
    with col1:
        st.subheader("Play probabilities:", anchor=False)
        plot_play_probabilities(nn_encoder.categories_[0], class_probabilities)

    pkl_path = Path("streamlit/sources/nn_regressor.pkl")
    with pkl_path.open("rb") as f:
        nn_regressor = dill.load(f)  # TODO: replace with joblib # noqa: S301
    st.divider()
    if st.session_state["choice"] == "Live Game":
        with col2:
        ...
        with col1:
            ...
            plot_win_probabilities(
                plays_df["totalTimeLeft"],
                win_probabilities[:, 0],
                plays_df["homeColor"].to_numpy()[0],
                plays_df["awayColor"].to_numpy()[0],
                plays_df["homeName"].to_numpy()[0],
                plays_df["awayName"].to_numpy()[0],
            )
        with col2:
        ...
        st.subheader("Win probabilities:", anchor=False)
        probabilities = nn_regressor.predict(
            pd.DataFrame.from_dict(prediction_data, orient="index").T,
        )
        ...
        def render_side(
            ...
            *,
            ...
            return (
                f'<div style="flex:{prob}; background-color:{color}; display:flex; '
        ...
        home_html = render_side(
            ...
            show_text=home_prob > 10,  # noqa: PLR2004
        )

        tie_html = render_side(prob=tie_prob, color="#444444", show_text=False)
        ...
        away_html = render_side(
            ...
            show_text=away_prob > 10,  # noqa: PLR2004
        )
...
if st.session_state["choice"] == "User Input (Tree)":
    with st.spinner("Planting Tree..."):
        pkl_path = Path("streamlit/sources/decision_tree_model.pkl")
        with pkl_path.open("rb") as f:
            clf = pickle.load(f)  # TODO: replace with joblib # noqa: S301
    ...
    class_probabilities = (
        tree.value[current_node].flatten() / tree.value[current_node].sum()
    )
    ...
    with col1:
        ...
        if (
            tree.children_left[current_node] == -1
            and tree.children_right[current_node] == -1
        ):
            st.success(
                f"The most likely next play is a &nbsp;&nbsp;&nbsp; **{clf.classes_[predicted_class]}** &nbsp;&nbsp;&nbsp; with a probability of &nbsp;&nbsp;&nbsp; **{round(class_probabilities[predicted_class] * 100)}%**",
            )
            ...
            st.markdown(
                f"Is&nbsp;&nbsp; <span class='custom-code'>{feature_name}</span> &nbsp; <= &nbsp; {threshold}?",
                unsafe_allow_html=True,
            )

            # Checkbox for each decision
            if st.checkbox(
                f"Yes:&nbsp;  {feature_name}  <=&nbsp; {threshold}",
                key=f"left_{current_node}",
            ):
                next_node = tree.children_left[current_node]
                st.session_state.path.append(
                    [feature_name, threshold, True, current_node],
                )
                st.session_state.current_node = next_node
                st.rerun()

            if st.checkbox(
                f"No:&nbsp;&nbsp;  {feature_name}  >&nbsp; {threshold}",
                key=f"right_{current_node}",
            ):
                next_node = tree.children_right[current_node]
                st.session_state.path.append(
                    [feature_name, threshold, False, current_node],
                )
                st.session_state.current_node = next_node
                st.rerun()
    ...
    if toggled:
        ...
        for step in st.session_state.path:
            st.markdown(
                f"{step[0]}&nbsp;&nbsp;&nbsp;<=&nbsp;&nbsp;&nbsp;{step[1]}&nbsp;?&nbsp;&nbsp; {':green-badge[:material/check: True]' if step[2] else ':red-badge[:material/close_small: False]'}",
            )
