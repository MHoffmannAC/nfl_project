...
def get_stats(game_id):
    ...
    stats = {}
    for i in [0,1]:
        stats[i] = {stat['name']: stat['displayValue'] for stat in response.json()['boxscore']['teams'][i]['statistics']}

    stats[0]['interceptions'], stats[1]['interceptions'] = stats[1]['interceptions'], stats[0]['interceptions']
    stats[0]['fumblesLost'], stats[1]['fumblesLost'] = stats[1]['fumblesLost'], stats[0]['fumblesLost']
...
def generate_top_games_posts(winners, year, week, games_df, teams_df):
    ...
    cover_img = create_cover_image("streamlit/images/tt_cover.png", "Top Games of the Week", year_week_text)
    st.image(cover_img)
...
def generate_game_stats_posts(year, week, games, teams_df):
    ...
    year_week_text = f"Season {year}/{year+1} - Week {week}"
    cover_img = create_cover_image("streamlit/images/tt_cover_red.png", "Game stats", year_week_text)
    st.image(cover_img)
    ...
    for game in games:
        ...
        home_abbreviation = teams_df.loc[teams_df['team_id'] == game['home_team_id'], 'abbreviation'].values[0]
        away_abbreviation = teams_df.loc[teams_df['team_id'] == game['away_team_id'], 'abbreviation'].values[0]
        
        home_color = teams_df.loc[teams_df['team_id'] == game['home_team_id'], 'color'].values[0]
        away_color = teams_df.loc[teams_df['team_id'] == game['away_team_id'], 'color'].values[0]
                
        title = f"Season {year}/{year+1} - Week {week}"
        subtitle = f"{away_abbreviation} @ {home_abbreviation}"

        social_post_img = create_image_from_stats(game, home_color, away_color, title, subtitle)
        ...
        social_post_img.save(img_buffer, format="PNG")
        img_buffer.seek(0)
        filename = f"{img_ind:02d}_{away_abbreviation}-{home_abbreviation}_week_{week}.png"
        images_to_zip.append((filename, img_buffer))
    ...
    if images_to_zip:
        ...
        st.download_button(
            label="📥 Download All Posts as ZIP",
            data=zip_buffer,
            file_name=f"game_stats_week_{week}.zip",
            mime="application/zip"
        )