...
def plot_play_probabilities(classes, probabilities):
    ...
    for i, bar in enumerate(bars):
        ax.text(
            bar.get_width() + 1
            if bar.get_width() < 90
            else bar.get_width() - 1,  # Position to the right of the bar
    ...
    for line in dot_data_lines:
        if "label=<" in line:
            ...
            new_label_parts = [
                ...
                if "value =" not in part and "samples =" not in part
            ]
            if (
                tree.children_left[node_id] == TREE_LEAF
                and tree.children_right[node_id] == TREE_LEAF
            ):
            ...
            new_line = (
                line.split("label=<")[0]
                + f"label=<{new_label}>, fontsize={font_size},"
                + line.split(">,")[1]
            )
            if (
                ...
                new_line = re.sub(
                    ...
                    'fillcolor="#333333", fontcolor="#eeeeee"',
                    new_line,
                )
