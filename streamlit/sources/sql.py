...
def update_week(week, season, game_type, sql_engine):
    ...
    if(len(games_in_db)>0):
        ...
        response = requests.get(url)
        data = response.json()
        games_df = load_game_data(data['content']['sbData']['events'], sql_engine, asDataFrame=True, checkExistence=False)
        if(len(games_df)>0):
        ...
        response = requests.get(url)
        data = response.json()
        games_df = load_game_data(data['content']['sbData']['events'], sql_engine, asDataFrame=True)
        if(len(games_df)>0):
            append_new_rows(games_df, 'games', sql_engine, 'game_id')
